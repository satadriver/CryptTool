...
// 编码函数
string MD5::encode(unsigned char * src,int len,unsigned char * dst) {
	...
	unsigned int* lpd = (unsigned int*)dst;
	lpd[0] = tempA;
	lpd[1] = tempB;
	lpd[2] = tempC;
	lpd[3] = tempD;
...
int GetMD5(unsigned char* data,int len, unsigned char* md5,int strFormat) {
	MD5 test;
	string strmd5 = test.encode(data,len,md5);
	if (strFormat) {
...
vector<unsigned int> MD5::padding(unsigned char * src,int len) {
	// 以512位,64个字节为一组

	unsigned int number = ((len + 8) / 64) + 1;
	vector<unsigned int> rec(number * 16);
	strlength = number * 16;
	for ( int i = 0; i < len; i++) {
